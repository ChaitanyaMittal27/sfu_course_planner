...
- Keep frontend auth responsibilities separate: `proxy.ts` is the route guard and session-refresh boundary; `AuthContext` mirrors browser-session state for UI only; login, signup, and callback handlers navigate only after their own successful action. Build provider callback URLs with `lib/auth/redirect.ts` so email confirmation and OAuth preserve the same validated internal `redirectTo` destination. Do not add client-side route guards that compete with the proxy.
