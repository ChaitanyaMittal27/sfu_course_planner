...
- Human-facing course links use the frontend-only canonical paths `/browse/[deptCode]/[courseNumber]` and `/browse/[deptCode]/[courseNumber]/[semesterCode]`. Build them with `lib/course-routes.ts`; resolve their code/number to internal IDs through `lib/course-resolver.ts` before calling the unchanged ID-based backend API. Keep legacy numeric URLs as compatibility entries, but generate only canonical URLs in new navigation.
