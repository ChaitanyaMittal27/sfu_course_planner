...
- Course offering details have one canonical, refreshable route: `/browse/departments/[deptId]/courses/[courseId]/offerings/[semesterCode]`. Keep Browse rows, Dashboard bookmarks, and post-auth redirects pointed there; the parent browse URL retains `dept` and `course` query parameters.
